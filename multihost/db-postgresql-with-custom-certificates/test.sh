...
set timeout 20
spawn keylime_tenant -v ${ATTESTATION_SERVER_IP} -t ${AGENT_IP} -u ${AGENT_ID} --allowlist /var/tmp/allowlist.txt --exclude /var/tmp/excludelist.txt --include payload-${REVOCATION_SCRIPT_TYPE} --cert default -c add
...
Agent() {
    rlPhaseStartSetup "Agent setup"
        ...
        limeCreateTestLists
        #copy allow list to tenant for keylime tenant add
        rlRun "scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i ~/.ssh/id_rsa_multihost allowlist.txt excludelist.txt root@$ATTESTATION_SERVER_IP:/var/tmp/"
